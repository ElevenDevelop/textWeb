(function ($) {
        ...
        target.getChildNodes = function (data, parentNode, parentIndex, tbody) {
            $.each(JSON.parse(data), function (i, item) {
                if (item[options.parentCode] == parentNode[options.code]) {
                    if(item.State==0){item.State="通过";}else{item.State="未通过"}
                    var tr = $('<tr></tr>');
        ...
        target.load = function (parms) {
            ...
            $.each(options.columns, function (i, item) {
                ...
                if (i == 0 && item.field == 'selectItem') {
                    hasSelectItem = true;
                    th = $('<th style="text-align:center;width:36px;vertical-align:middle"><input name="select_item_all" type="radio"></input></th>');
                } else {
            ...
            var thead = $('<thead class="treegrid-thead" style="text-align:center"></thead>');
            thead.append(thr);
            target.append(thead);
            // 构造表体
            ...
            tbody.html(_loading);
            // 默认高度 parms ? parms : options.ajaxParams,
            if (options.height) {
            ...
            $.ajax({
                type: options.type,
                url: options.url,
                data: JSON.stringify(options.ajaxParams),
                ...
                success: function (data, textStatus, jqXHR) {
                    ...
                    if(options.ajaxParams.option==""||options.ajaxParams.serchdata==""){
                        ...
                        if (hasSelectItem) {
                            ...
                            if (_ipt.attr("type") == "radio") {
                                if (_ipt.prop('checked')) {
                                    ...
                                    $("#DescriptionCom").val("");
                                    if($("#role").text()=="管理员"||$("#role").text()=="系统用户"){
                                        $("#DescriptionCom").show();
                                        $("#titlemanger").show();
                                        $("#DescriptionManger").hide();
                                    ...
                                    $.ajax({
                                        type: "post",
                                        url: "http://192.168.6.25:1531/api/HomeData/GetDescription",
                                        data: JSON.stringify({Id: chk_value[0].Id}),
                                        contentType: "application/json",
                                        dataType: "json",
                                        async: true,
                                        success: function (data) {
                                            ...
                                            //修改
                                            if(json.ParentId==0){
                                                $("#DescriptionCom").val(json.Id);
                                                $("#DescriptionCom").attr("disabled","disabled");
                                                $("#childName").hide();
                                                $("#Description").hide();
                                                $("#Description").val("");
                                              if($("#role").text()=="管理员"||$("#role").text()=="系统用户"){
                                                  $("#DescriptionCom").hide();
                                                  $("#titlemanger").hide();
                                                  $("#DescriptionManger").show();
                                                  $("#DescriptionManger").val(json.Description);
                                              }
                                            }else{
                                                $("#titlemanger").show();
                                                $("#DescriptionCom").show();
                                                $("#DescriptionCom").attr("disabled","disabled");
                                                $("#DescriptionCom").val(json.ParentId);
                                                $("#DescriptionManger").hide();
                                                $("#DescriptionManger").val("");
                                    ...
                                    //修改
                                    $("#DescriptionId").val(chk_value[0].Id);
                                    //删除
                                ...
                                //     $("#DescriptionParent").attr("disabled", false);
                                //     $(this).removeClass("treegrid-selected");
                                // } else {
                                //     _ipt.prop('checked', true);
                                //     var chk_value = [];
                                ...
                                //             var json = eval("(" + data + ")");
                                //             if (json.handleState) {
                                //                 $("#DescriptionParent").val(json.DescriptionParent);
                                //                 $("#DescriptionParent").attr("disabled", "disabled");
                                //             }
                                //         },
                                //         error: function (data) {
                                //             alert(+data.status + " : " + data.statusText + " : " + data.responseText);
                    ...
                    target.find("thead").find("input[name='select_item_all']").click(function (event) {
                        ...
                        target.find('tbody > tr').each(function () {
                            var row = this;
                            var td_check = $(this).find('input[type=radio]');
                            if (th_checked) {
    ...
    $.fn.bootstrapTreeTable.methods = {
        ...
        // 刷新记录
        refreshs: function (target, parms) {
            if (parms) {
                target.load(parms);
            } else {
                target.load();
